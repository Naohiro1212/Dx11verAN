...
LevelUpEffect::LevelUpEffect(GameObject* parent, XMFLOAT3 pos) : GameObject(parent, "LevelUpEffect"),
timer_(0.0f), data_(), hEmit_(-1)
{
	transform_.position_ = pos;
}

void LevelUpEffect::Initialize()
{
	// 敵が死んだ時のエフェクトを流用し、黄色っぽく変更
	...
	data_.color = XMFLOAT4(1.0f, 0.9f, 0.2f, 0.85f); // 黄色＋高α
	data_.deltaColor = XMFLOAT4(-0.001f, -0.001f, -0.001f, -0.012f); // 徐々に暗く＆フェードアウト

	data_.size = XMFLOAT2(2.8f, 2.8f);   // 円形に近い
	...
	data_.gravity = -0.0050f;   // 上昇を徐々に止める（落下はしない程度）
...
void LevelUpEffect::Draw()
{
}

void LevelUpEffect::Release()
{
}
