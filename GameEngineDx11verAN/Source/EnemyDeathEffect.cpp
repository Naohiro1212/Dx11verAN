...
void EnemyDeathEffect::Initialize()
{
	...
	data_.position = { transform_.position_.x, transform_.position_.y + 10.0f, transform_.position_.z };
	data_.positionRnd = XMFLOAT3(0.6f, 0.2f, 0.6f);   // 少し広げて発生

	data_.delay = 10;        
	data_.number = 30;       // 密度（多め）
	data_.lifeTime = 140;      // 長めに残す（フレーム）
	...
	data_.gravity = 0.0035f;   // 上昇を徐々に止める（落下はしない程度）
